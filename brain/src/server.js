...
app.post("/server", async function (req, res) {
  ...
  let status = updateConfig(req.body);
...
app.get("/params", async function (req, res) {
  ...
  try {
    URL = getProperty("origin", "");
    var params = getOriginalasJSON(URL + "/params.json");
    var html = getOriginalasText(URL);
